...
function Tv1() {
    ...
    const handleDelete = async (filename) => {
        try {
            const response = await axios.delete(`${BaseUrl}/videos/${filename}`);
    ...
    return (
        <Container maxWidth="md">
            ...
            <Button
                sx={{marginLeft: '10px'}}
                variant={"contained"}
                onClick={()=>{navigate('/tv2');}}
            >Tv2 Ye GEÇ</Button>
            <Button
                sx={{marginLeft: '10px'}}
                variant={"contained"}
                onClick={()=>{navigate('/tv3');}}
            >Tv3 e GEÇ</Button>
            <Button
                variant={"contained"}
                sx={{marginLeft: '10px'}}
                onClick={()=>{navigate('/tv4');}}
            >Tv4 e GEÇ</Button>
            <Button
                sx={{marginLeft: '10px'}}
                variant={"contained"}
                onClick={()=>{navigate('/tv5');}}
            >Tv5 e GEÇ</Button>
            <Grid item >
                <Button
                    sx={{ marginTop: '10px' }}
                    variant={"outlined"}
                    onClick={() => {window.location.href = 'https://garagestreet.com.tr';}}
                >
                    Televizyon 1 i İzle
                </Button>
            </Grid>
